  ...
  const onStarRestaurant = async (id) => {
    try {
      ...
      if (status !== 200 && status !== 201) {
        alert("Failed to star the restaurant. Status code: " + status);
        return;
      }

      dispatch({ type: "STAR_RESTAURANT", payload: data });
    } catch (error) {
      console.error("An error occurred while starring the restaurant:", error);
      alert(`Error: ${error.message} while starring the restaurant.`);
    }
  };
